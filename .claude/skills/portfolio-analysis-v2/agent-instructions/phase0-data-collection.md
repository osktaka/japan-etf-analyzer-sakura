...
- **リトライロジック（必須）**: 配当データ取得は以下の2段階で行う:

  **第1段階: 一括取得**
  1. 第1試行: `AnalysisDataService().get_analysis_data(etf_codes)` で全銘柄の配当データを一括取得
  2. 成功した場合: 第2段階をスキップし、正常完了

  **第2段階: 個別リトライ（第1段階失敗時）**
  3. 第1段階が失敗（JSONパースエラー/タイムアウト等）した場合: **3秒待機**
  4. 個別銘柄ごとに `get_analysis_data([single_code])` で1銘柄ずつ取得を試行
  5. 個別取得でも失敗した銘柄: ticker形式を変更して再試行（例: `1475.T` → `1475`、またはその逆）
  6. 個別でも2回失敗した銘柄: `_data_status.dividend_data.per_etf.{code}` = `{"status": "error", "reason": "個別リトライ後も取得失敗"}` を記録。該当銘柄のdividend_dataを `null` に設定
  7. Phase 1の会議では「配当データ未取得」として扱い、**推測値での分析は禁止**する

- **フォールバック**: 全銘柄で個別リトライ後も失敗した場合は `dividend_data: null` を設定し、`_data_status` に `{"status": "error", "error": "配当データ取得失敗（一括+個別リトライ済み）"}` を記録。この場合、Phase 0.5の分配金Zスコア（項目10）は一括スキップされる
