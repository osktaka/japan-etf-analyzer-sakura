...
6.4. 取引回避日チェック: `0b_trend_summary.md` の「取引回避日判定」を確認し、「回避推奨」の場合はセクション10.5の冒頭に「⚠ 本日はSQ日のため新規取引を回避推奨。緊急性の高い損切りのみ実行対象」と注記する
6.5. セクション10.5（取引実行判定）を生成: セクション10の全Phase提案を走査し、以下の基準で即時実行対象を判定する
   - Phase 1の売買提案（売却/購入/入替/増額/縮小）: 市場環境フィルター通過後に実行対象候補
     - 市場環境フィルター（`0a_market_environment.md` を参照）:
       a. 日経VI（または相当指標）が前日比+30%以上 → 買い提案は1営業日待機（売り提案は通過）
       b. 対象銘柄の出来高が過去20日平均の50%未満 → 流動性不足で保留
       c. フィルター非該当 → 実行対象候補として通過
     - フィルターで保留となった提案は「市場環境フィルターにより保留」と理由を付記
...
6.6. 進行中計画の照合（HISTORY.mdに進行中の売買計画がある場合のみ実行）:
   ...
   c2. 緊急トリガーがある計画は、通常トリガーに先立ち緊急トリガーを評価する。緊急トリガー充足時は通常トリガーの段階を飛ばして緊急アクションを即時実行対象に追加する
