...
### 処理時間チェックポイント（30分目標の達成検証）

各Stepの完了時にメインエージェントが経過時間を確認し、超過時は次Phaseの短縮指示を出す。

| チェックポイント | 目標経過時間 | 超過時の対応 |
|----------------|------------|------------|
| Step 1完了（Phase 0 + 0a + 0b） | 8分以内 | Step 2のタイムアウトを7分に短縮。Phase 0.5の追加計算（項目8-13）で低優先項目をスキップ |
| Step 2完了（Phase 0.5） | 13分以内 | Step 3の各会議タイムアウトを6分に短縮。会議の議論ラウンドを3→2に削減指示 |
| Step 3完了（Phase 1全会議） | 23分以内 | Step 4のマージ会議で反論ラウンドを省略。Step 5のレポート生成で補足資料（セクション13）を簡略化 |
| Step 4完了（Phase 2） | 27分以内 | Step 5のレポートでセクション9/11/13を最小構成にする |
...
### Phase 0.5 部分失敗時の続行判定

| 失敗した計算項目 | 続行可否 | 理由 |
|----------------|---------|------|
| シャープレシオ（項目1） | ❌ 中止 | Phase 1全会議で必須参照 |
| 相関分析（項目2） | ⚠️ 条件付き続行 | 会議2のみ影響。会議1・3は続行可 |
| 最大DD（項目3） | ⚠️ 条件付き続行 | Phase 1で定性議論に切替え |
| VaR/CVaR（項目7） | ⚠️ 条件付き続行 | ストレスシナリオ分析で代替 |
| HHI（項目H） | ✅ 続行 | 補助指標。セクター/地域比率で代替 |
| CF-VaR（項目CF） | ✅ 続行 | パラメトリックVaRで代替 |
| 通貨エクスポージャー（項目CX） | ✅ 続行 | 補助指標 |
| ファクター分析（項目FA） | ✅ 続行 | 補助指標 |
| テクニカル指標（項目8-13） | ✅ 続行 | 会議議論の補助のみ |

**部分失敗時の処理**:
1. `_calc_temp.json` の各キーの `status` を確認
2. 上記テーブルで ❌ の項目が失敗 → スキル全体を中止
3. ⚠️ の項目が失敗 → 該当会議に「{項目名}データ不足」の追加パラメータを渡す
4. ✅ の項目が失敗 → 警告のみで続行
