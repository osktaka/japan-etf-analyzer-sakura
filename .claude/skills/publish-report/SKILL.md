...
- `/portfolio-analysis` スキルの出力レポート（600行超）を、一般読者向けの2,500-3,000字のノート記事（通常回）に変換
...
1. `article-template.md` に従い2,500-3,000字の記事を生成（通常回）
...
- **「マイページ（デモ）」への誘導**: 次のステップでデモポートフォリオの確認先として言及する
...
- レポートの数値を正確に引用している
- 2,500-3,000字の範囲内（通常回）、1,800-2,200字（初回）
