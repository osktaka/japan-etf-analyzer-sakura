...
| モード | ラベル | 説明 | 所要時間目安 |
|-------|-------|------|------------|
| speed | 速度重視 | タスクを分割して並行実行。クロスレビューなし。最短時間で結果を得る | 8-15分 |
| normal | ノーマル（推奨） | タスク分割＋並行実行＋クロスレビュー1回。バランス重視 | 13-20分 |
...
Phase 0a + Phase 0 + Phase 0b（並行）→ Phase 0.5（共通定量計算）→ Phase 1（タスク分割型並行、3エージェント別タスク）→ Phase 2: スキップ → Phase 3+4（統合、クロスレビューなし）

- Phase 0.5で基礎計算（シャープレシオ、相関、DD、VaR等）を事前実行。Phase 1は計算結果を引用して解釈に集中
- クロスレビューを完全スキップ。セクション9は「速度重視モードのためスキップ」と記載
- Phase 2（入替候補の外部検証）はスキップ

#### ノーマル（normal）

Phase 0a + Phase 0 + Phase 0b（並行）→ Phase 0.5（共通定量計算）→ Phase 1（タスク分割型並行、3エージェント別タスク）→ Phase 2（入替候補の外部検証）→ Phase 3+4（統合、クロスレビュー含む）

- Phase 0.5で基礎計算を事前実行（debateモードと同一処理）。Phase 1は計算結果を引用して解釈に集中
...
- （全モード共通）Phase 0.5で決定論的な計算結果（シャープレシオ、相関係数、ドローダウン等）および追加テクニカル指標（`phase05b-trader-indicators.md` 参照: 200MA、ATR、Zスコア、出来高、再購入スコア、経済象限）を `05_shared_calculations.md` に出力
...
### Phase 0.5: 共通定量計算（全モード共通）

**開始条件**: Phase 0が完了した時点で即起動する（Phase 0aの完了を待たない。リスクフリーレートはフォールバック値0.5%を使用）。
**実行条件**: 全モード（speed/normal/debate）で実行する。

決定論的な計算（シャープレシオ、相関係数、ドローダウン等）を1エージェントで実行し、Phase 1の各アナリストエージェントが計算を重複実行することを防ぐ。
...
**開始条件**: Phase 0aとPhase 0の**両方が完了**してからPhase 1を起動する。全モードでPhase 0.5の完了も必要（Phase 0aはPhase 0.5と並行実行中でよい）。
...
- [ ] テクニカル指標セクション（6.3分配金利回り、7.2防御シグナル、7.3出来高）がレポートに存在する、またはデータ不足によるスキップ理由が記載されている（全モード共通でPhase 0.5が実行されるため、Phase 1アナリストは計算結果を引用する）
