...
- 見解の相違点とその根拠
- 合意された点
- 各アクションの視点一致度（5段階: 5/5, 4/5, 3/5, 2/5, 1/5以下）
...
**視点一致度の記載方法**:
- 各分析セクションの結論部分に `【視点一致度】` を付記する
- 形式: `【視点一致度: 5/5, 4/5, 3/5, 2/5, 1/5以下】`

| 合意数 | 視点一致度 | 扱い |
|--------|-------|------|
| 5/5 | 5/5 | 高確信度で推奨 |
| 4/5 | 4/5 | 強い推奨（反対1名の懸念を付記） |
| 3/5 | 3/5 | 条件付き推奨（反対2名の懸念を併記） |
| 2/5 | 2/5 | 見解分かれる（各見解併記、読者に判断委ねる） |
| 1/5以下 | 1/5以下 | 非推奨（提案者の根拠のみ記録） |

**5ペルソナの視点一致度を統合する手順**:
1. 各アクションについて、各ペルソナ（A/B/C/D/E）の賛否を `31_crossreview_*.md` と `32_crossreview_*.md` から確認する
2. 賛成数を集計し、上記テーブルの視点一致度を決定する
...
4a. セクション10（最適化提案）の各入替提案に外部検証結果を反映する:
   - `{WORK_DIR}/20_candidate_verification.md` が存在する場合: 各入替提案テンプレートの **視点一致度** 行の後に `**外部検証**: {判定}（{リスク付記}）` 行を追加する。判定・リスク付記は 20_candidate_verification.md の各候補の検証結果から転記する
...
7d. **品質ゲートチェック（必須）**: レポート出力前に以下の品質ゲートを検証し、結果をセクション15の品質ゲートテーブルに記載する。

    | 検証項目 | speed | normal | debate | 判定基準 |
    |---------|-------|--------|--------|---------|
    | リスクフリーレート出典 | o | o | o | 取得元URLが mof.go.jp または boj.or.jp であること。それ以外はフォールバック0.5%が使用されていること |
    | VaR注釈のデータ点数記載 | o | o | o | VaR/CVaRセクションにデータ点数と信頼度が記載されていること |
    | 免責事項セクション | o | o | o | レポート冒頭（エグゼクティブサマリーの前）に免責事項セクションが存在すること |
    | 視点一致度の分数表記 | - | - | o | 「視点一致度 X/5」の分数表記が使われていること |
    | 前回レポートとのRf差 | o | o | o | `reports/{username}/metrics.json` の直近エントリのリスクフリーレートと比較。差が0.5%超の場合は「前回比変動: X.XX% → Y.YY%」と警告記載 |

    **判定結果の表記**:
    - OK: 検証合格
    - 警告: 基準を満たさないが続行可能（警告内容を備考に記載）
    - N/A: 該当モードでは検証対象外
    - エラー: 基準未達（レポートに警告バナーを挿入）

7e. **サマリー↔詳細の整合性チェック（必須）**: セクション1のサマリー評価と、セクション5-9の詳細分析が矛盾しないことを確認する。

    - [ ] サマリーで「分散良好」と評価している場合 → セクション5で相関係数0.85超の「実質重複」ペアが存在しないこと
    - [ ] サマリーで「リスク管理良好」と評価している場合 → セクション6でVaR警告レベルの指摘がないこと
    - [ ] サマリーの総合評価トーン（ポジティブ/ネガティブ）と、詳細分析（セクション5-9）の個別指摘が方向性で矛盾しないこと
    - **不一致があった場合**: サマリーの評価表現を詳細分析の結果に合わせて修正する（詳細分析の数値が正とする）

7f. **LLM自由記述の妥当性チェック（必須）**: セクション10の「効果」「定量目標」およびセクション11.1の「改善後」予測値について、過度に楽観的な記述がないか確認する。

    - [ ] 改善後シャープレシオが現状の2倍超の場合 → 根拠（具体的な銘柄入替・比率変更とその影響試算）が明記されていること。根拠なしの場合は現状×1.5を上限として修正する
    - [ ] 改善後コストが現状の50%未満の場合 → 具体的な代替銘柄の信託報酬が記載されており、その値から計算して妥当であること
    - [ ] 定量目標が保守的推定（×0.75）ルールに従っていること → 理論上の改善幅に対し75%の達成率で目標が設定されていること
    - [ ] セクション10の各提案に「根拠チェーン」（データ→計算結果→解釈→提案理由の4段）が記載されているか。記載がない提案は根拠チェーンを追記すること
    - **基準超過があった場合**: 根拠が不十分なら保守的な値に修正し、修正した旨をセクション15の品質ゲートテーブルに「警告」として記載する

7g. **Phase 0.5→レポートの数値トレーサビリティチェック（debateモード必須）**: `05_shared_calculations.md` に記載された定量計算結果がレポートにそのまま転記されていることを確認する。

    - [ ] シャープレシオ値（各銘柄・ポートフォリオ加重）が `05_shared_calculations.md` の値と一致すること
    - [ ] 相関係数（各ペア）が `05_shared_calculations.md` の値と一致すること
    - [ ] 最大ドローダウン値が `05_shared_calculations.md` の値と一致すること
    - [ ] VaR/CVaR値が `05_shared_calculations.md` の値と一致すること
    - [ ] 計算メタデータ（リスクフリーレート値、市場代理ETF）が `05_shared_calculations.md` の記載と一致すること
    - **不一致があった場合**: レポートの数値を `05_shared_calculations.md` の値で上書きする（Phase 0.5のプログラマティック計算結果が正とする）
    - **normalモードの場合**: `10_quant_analysis.md` / `10_score_analysis.md` の数値がソースとなるため、このチェックはスキップする
...
エグゼクティブサマリーの「ヘルスチェック」テーブルを生成するため、以下のロジックで判定する。

### データソース
- `05_shared_calculations.md` の計算結果を使用
- Phase 0.5が未実行の場合はPhase 1の分析結果から推定

### 判定基準

| 観点 | 🟢 緑 | 🟡 黄 | 🔴 赤 | データソース |
|------|-------|-------|-------|------------|
| 総合健全性 | スコア ≥ 70 | スコア ≥ 50 | スコア < 50 | score_cacheの加重平均（balance視点） |
| リスク水準 | 最大DD ≥ -15% | 最大DD ≥ -25% | 最大DD < -25% | max_drawdown計算結果 |
| 分散度 | 最大セクター比率 ≤ 30% | ≤ 40% | > 40% | tag_dataのセクター集計 |

### 判定不可の場合
- データ不足で判定できない観点は「⚪ データ不足」と表示
- 5銘柄未満で分散度判定不可の場合は「⚪ 5銘柄未満のため判定省略」
...
Phase 1の内部分析ファイル（10_*.md）では率直な表現（SELL、推奨等）を許容するが、**最終レポートへの転記時**に以下のトーン変換を行う:
