...
- 見解の相違点とその根拠
- 合意された点
- 各アクションの方向性一致度（5段階: 5/5, 4/5, 3/5, 2/5, 1/5以下）
...
**方向性一致度の記載方法**:
- 各分析セクションの結論部分に `【方向性一致度】` を付記する
- 形式: `【方向性一致度: 5/5, 4/5, 3/5, 2/5, 1/5以下】`
...
4a. セクション10（最適化提案）の各入替提案に外部検証結果を反映する:
   - `{WORK_DIR}/20_candidate_verification.md` が存在する場合: 各入替提案テンプレートの **方向性一致度** 行の後に `**外部検証**: {判定}（{リスク付記}）` 行を追加する。判定・リスク付記は 20_candidate_verification.md の各候補の検証結果から転記する
...
7d. **品質ゲートチェック（必須）**: レポート出力前に以下の品質ゲートを検証し、結果をセクション15の品質ゲートテーブルに記載する。
    ...
    | 免責事項セクション | o | o | o | レポート末尾に免責事項セクションが存在すること |
    | 方向性一致度の分数表記 | - | - | o | 「方向性一致度 X/5」の分数表記が使われていること |
