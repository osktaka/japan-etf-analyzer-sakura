...
> **追加データ**: `portfolio_data.json` の `compare_performance` と `compare_scores` があれば、分析間の矛盾検証の根拠として参照すること。
> **外部検証データ**: `{WORK_DIR}/candidate_verification.md` が存在する場合は、クロスレビュー時に参照すること。入替提案の妥当性を外部検証結果と照合して確認し、「要再検討」「非推奨」と判定された候補については検証結果の根拠をレビューに反映する。
