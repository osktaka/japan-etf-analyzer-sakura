...
| 微小変動閾値 | ±0.1% | 判定不能の基準（Phase 3で使用） |
| R1タイムアウト | 180秒 | 妥当性チェック |
| データ収集タイムアウト | 300秒 | サブエージェント全体 |
| 時間帯判定境界 | 15:30 JST | AM/PM自動判定 |

※ 各パラメータの詳細は本文中の該当セクションを参照。値を変更する場合はこの一覧と本文の両方を更新すること。
...
3. ドラフト生成（メイン）※未保存
   ...
   ├─ AM: 見通し判断（上昇/下落/横ばい + データ整合性）
   └─ PM: 結果まとめ + AM予想との比較
   　　　├─ AMファイル（reports/market-outlook/YYYYMMDD_am.md）を読み込みメタデータをパース
   　　　│　　※ AMファイル先頭がYAMLフロントマターでない場合、末尾の`<!-- prediction: ... -->`からパースを試みる（後方互換）
   　　　├─ AMファイルなし: 比較セクション省略
   　　　└─ パース失敗: 「AM予想データの読み取り失敗」と明記
        ↓
4. レビュー実施判定（メイン）
   ├─ --no-review → Step 6へ直行（<!-- review_skipped: user_opt_out -->）
   ※ レビューはAM実行有無・営業日/非営業日に依存せず実施する（AM比較セクションのみAMファイル不在時は省略）
        ↓
...
タイムアウト: 300秒（データ収集サブエージェント全体。超過時は取得済みデータで出力を構成する）
```
...
パラメータマッピング:
- {{shared_data}} = Step 3ドラフトから以下を除外した内容: ソース一覧セクション、YAMLフロントマター（prediction/review）、保有銘柄データセクション（portfolioモード時の生JSON）
...
- `prediction.counter_scenario`: 反対シナリオ1文（レビュー実施時のみ）
- `review.applied`: 改善要約1行（レビュー実施時のみ）

メタデータ分割基準: 構造化データ（prediction, review結果）= YAMLフロントマター / 運用フラグ（review_skipped, sector_data_unavailable等）= HTMLコメント（本文末尾に残す）
...
4. 改善禁止: 指標数値・ソース一覧・メタデータの変更禁止
   ※ データ整合性の見直しが合意された場合のみメタデータ更新可
5. YAMLフロントマターの `review.applied` に改善要約1行を記録する
...
- [ ] [AM] YAMLフロントマター（prediction/review）がファイル先頭に含まれている
...
- [ ] [AM/PM共通] 改善版ドラフトのYAMLフロントマターに `review.applied` が記録されている（AM）、または本文末尾に `<!-- review_applied: ... -->` が含まれている（PM）
