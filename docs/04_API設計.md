...
| メソッド | パス | 機能 | 対応機能ID | 認証 |
|----------|------|------|------------|------|
| GET | /demo/portfolio | デモポートフォリオ概要取得 | F-105 | 不要 |
| GET | /demo/portfolio/holdings | デモ保有銘柄一覧取得 | F-105 | 不要 |
| GET | /demo/portfolio/valuation-history | デモ評価額推移取得 | F-105 | 不要 |
| GET | /demo/favorites | デモお気に入り一覧取得 | F-102 | 不要 |
| GET | /demo/trades | デモ取引履歴取得 | F-103 | 不要 |
| GET | /demo/cash-flows | デモ入出金一覧取得 | F-103 | 不要 |

**備考:**
- 全エンドポイントが認証不要（未ログインユーザー向けのプレビュー機能）
