...
### 計算前必須チェック（Claude Code 全タスク共通ルール）

**価格・損益・取引数量・評価額・パフォーマンス・シミュレーション・バックテストなど、株価関連の計算を行う前に必ず以下を確認すること。スキル経由か単発プロンプトかを問わず、このプロジェクトで「過去株価」「保有資産」「損益」「リターン」が話題になった瞬間にこのチェックは発動する。**

1. **対象銘柄に `stock_splits` レコードが存在するか確認する**（`is_applied` / `is_chart_applied` の値を把握）
2. **データ取得は必ずAPI経由とする**（`/api/v1/portfolio/*`, `/api/v1/chart/*` 等。内部実装は `PortfolioService` / `ChartService`）
3. **SQLite直接クエリで `trades.quantity` / `trades.unit_price` / 価格時系列を取得して計算に使うことを禁止**（DBの生データは分割前の元値のため、必ず不整合が出る）
4. **データソース混在を禁止**（API経由とDB直接クエリを同じ計算内で混ぜない）
5. **取得後の検証**: 計算結果が直感と乖離した場合、まず分割未調整を疑い、API出力と照合する

**過去事例**: 2026-02-14、Phase 0 のデータ収集で1615/1489/1597が分割調整前の数量・取得単価で算出され、レポート全体（記事化含む）を破棄する事態が発生。詳細は auto-memory 参照。
