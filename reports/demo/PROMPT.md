...
1. **スナップショット保存**: 更新前に、当日のスナップショットが無ければ現在の `HISTORY.md` を `history/YYYYMMDD.md` として保存する（過去のスナップショットは変更・削除しない）
2. **バックアップ**: 更新前に `HISTORY.md` を `HISTORY.md.bak` にコピーする（互換性のため残す）
3. **分析履歴テーブル**: 新しい行を追記
4. **現在のポートフォリオ**: 最新の保有状態で上書き
5. **未実行アクション**: 今回の提案で更新（実行済みは削除、新規提案を追加）
